...
import { OtherAssetRow } from '../lib/types';
...
export default function OtherAssetsTab({ rows, sheetTotal, dispatch }: Props) {
  return (
    <div className='space-y-4'>
            ...
            {rows.map((row) => (
              ...
              >
                ...
                <td className='border border-[#2A2A3A] px-3 py-2'>
                  <button
                    type='button'
                    onClick={() => dispatch({ type: 'DELETE_ROW', sheet: 'otherAssets', rowId: row.id })}
      ...
      <button
        type='button'
        onClick={() => dispatch({ type: 'ADD_ROW', sheet: 'otherAssets' })}
