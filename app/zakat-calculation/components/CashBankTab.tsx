...
import { CashBankRow } from '../lib/types';
...
export default function CashBankTab({ rows, sheetTotal, dispatch }: Props) {
  return (
    <div className='space-y-4'>
        ...
        <table className='min-w-190 border-collapse text-sm'>
          <thead className='bg-[#1E1E2E] text-[12px] uppercase tracking-[0.05em] text-[#B7B7C7]'>
            <tr>
              ...
              <th className='border border-[#2A2A3A] px-3 py-2 text-left'>Total Value</th>
          ...
          <tbody>
            {rows.map((row) => (
              ...
              >
                ...
                <td className={`border border-[#2A2A3A] px-2 py-2 ${row.isSectionHeader ? 'bg-[#1E1E2E] font-semibold text-[#D7D7E7]' : ''}`}>
                  <CellInput
                    value={row.description}
                    disabled={row.isSectionHeader}
                    onChange={(value) =>
                      dispatch({
                        type: 'UPDATE_ROW',
                        sheet: 'cashBank',
                ...
                <td className='border border-[#2A2A3A] px-3 py-2'>
                  <button
                    type='button'
                    onClick={() => dispatch({ type: 'DELETE_ROW', sheet: 'cashBank', rowId: row.id })}
                    disabled={row.isSectionHeader}
                    className='text-lg leading-none text-[#FF6B6B] transition hover:text-[#FF8989] disabled:cursor-not-allowed disabled:opacity-40'
                  >
                    ×
                  </button>
                </td>
      ...
      <button
        type='button'
        onClick={() => dispatch({ type: 'ADD_ROW', sheet: 'cashBank' })}
